...
#pragma once
#ifndef PREONMATH_EULER_H
#define PREONMATH_EULER_H
...
namespace Preon
{
    namespace Math
    {
        ...
        public:
            euler() : euler(0, 0, 0) { }
            euler(T phi, T theta, T psi) : m_Data(phi, theta, psi) { }
            explicit euler(const vec<3, T>& vector) : m_Data(vector) { }
            template <typename Real2> explicit euler(const euler<Real2>& e) : m_Data((T)e.phi(), (T)e.theta(), (T)e.psi()) {}
