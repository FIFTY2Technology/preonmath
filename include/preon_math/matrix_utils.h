...
#pragma once
#ifndef PREONMATH_MATRIX_UTIL_H
#define PREONMATH_MATRIX_UTIL_H
...
namespace Preon
{
    namespace Math
    {
        ...
        //        return;
            ...
            template<typename Scalar>
            matrix<4, 4, Scalar> worldTransform(const vec<3, Scalar>& position,
                                                const quat<Scalar>& orientation,
                                                const vec<3, Scalar>& scaleVector)
            {
            ...
            matrix<4, 4, Scalar> worldTransform(const vec<3, Scalar>& position,
                                                const quat<Scalar>& orientation,
                                                const vec<3, Scalar>& scaleVector,
                                                const vec<3, Scalar>& pivot, bool applyScaleOnPivot)
            {
                // Build the world transformation.
            ...
            inline void computeMaxEigenValue(const matrix<3, 3, float_simd>& m, float_simd& eigenMax, vec_simd<3>& eigenVector)
            {
                ...
                for (int i = 0; i < 20; i++)
                {
                    eigenVector = m * eigenVector;
                    eigenMax = eigenVector.length();
                    eigenVector = vec_simd<3>::and_mask_scalar(eigenVector / eigenMax, eigenMax > zero);
                }
            }
