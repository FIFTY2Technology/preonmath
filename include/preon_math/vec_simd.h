...
#pragma once
#ifndef PREONMATH_VEC_SIMD_H
#define PREONMATH_VEC_SIMD_H
...
namespace Preon
{
    namespace Math
    {
        ...
        namespace Simd
        {
            ...
            template<size_t D>
            PREONMATH_FORCEINLINE void setVecs(typename std::enable_if<D != 3, vec<3, float> >::type* out, const vec<D, float>& v0, const vec<D, float>& v1, const vec<D, float>& v2, const vec<D, float>& v3, const vec<D, float>& v4, const vec<D, float>& v5, const vec<D, float>& v6, const vec<D, float>& v7)
            {
                StaticFor<0, D>( [&] (size_t d) { (*out)[d] = _mm256_setr_ps(v0[d], v1[d], v2[d], v3[d], v4[d], v5[d], v6[d], v7[d]); });
            }

            //! Vec3f optimized version.
            PREONMATH_FORCEINLINE void setVecs(vec_simd<3>* out, const vec<3, float>& v0, const vec<3, float>& v1, const vec<3, float>& v2, const vec<3, float>& v3, const vec<3, float>& v4, const vec<3, float>& v5, const vec<3, float>& v6, const vec<3, float>& v7)
            {
                __m128 xy0, xy1;
            ...
            PREONMATH_FORCEINLINE void setVecs(vec_simd<3>* out, const Getter& func, uint numElements, const vec<3, float>& fillValue)
            {
                if (numElements > 4)
                {
                    setVecs_1(out, func(0),
                            func(1),
                            func(2),
                            func(3));
                    setVecs_2(out, func(4),
                ...
                else
                {
                    ...
                    StaticFor<0, 3>( [&] (size_t d)
                    {
                        (*out)[d] = _mm256_insertf128_ps((*out)[d], _mm_broadcast_ss(&fillValue[d]), 1);
                    });
            ...
            PREONMATH_FORCEINLINE void setVecArray(vec_simd<3>* out, const float* vecs)
            {
            ...
            template<size_t D>
            PREONMATH_FORCEINLINE void getVecs(const vec_simd<D>& input, vec<D, float>& v0, vec<D, float>& v1, vec<D, float>& v2, vec<D, float>& v3, vec<D, float>& v4, vec<D, float>& v5, vec<D, float>& v6, vec<D, float>& v7)
            {
                float buffer[8];
            ...
            template<size_t D>
            PREONMATH_FORCEINLINE void setVecs(typename std::enable_if<D != 3, vec_simd<D, float> >::type* out, const vec<D, float>& v0, const vec<D, float>& v1, const vec<D, float>& v2, const vec<D, float>& v3)
            {
            ...
            // See https://software.intel.com/en-us/articles/3d-vector-normalization-using-256-bit-intel-advanced-vector-extensions-intel-avx.
            PREONMATH_FORCEINLINE void setVecArray(vec_simd<3, float>* out, const float* vecs)
            {
                __m128 x0y0z0x1 = _mm_loadu_ps(vecs);
                __m128 y1z1x2y2 = _mm_loadu_ps(vecs + 4);
                __m128 z2x3y3z3 = _mm_loadu_ps(vecs + 8);
                ...
                (*out)[0] = _mm_shuffle_ps(x0y0z0x1, x2y2x3y3, _MM_SHUFFLE(2, 0, 3, 0));
                (*out)[1] = _mm_shuffle_ps(y0z0y1z1, x2y2x3y3, _MM_SHUFFLE(3, 1, 2, 0));
                (*out)[2] = _mm_shuffle_ps(y0z0y1z1, z2x3y3z3, _MM_SHUFFLE(3, 0, 3, 1));
            }
            ...
            PREONMATH_FORCEINLINE void setVecArray(vec_simd<3, double>* out, const double* vecs)
            {
                setVecs(out, [vecs](int i) { return vec<3, double>(vecs[i * 3], vecs[i * 3 + 1], vecs[i * 3 + 2]); });
            }
            ...
            {
                vec<D, double> out;
                for (size_t d = 0; d < D; ++d) out[d] = hSum(v[d]);
                return out;
            }
            ...
            {
                vec_simd<D, T> out;
                StaticFor<0, D>([&mask, &v1, &v2, &out](int d)
                {
                    out[d] = blendv(v2[d], v1[d], mask[d]);
                });
                return out;
            }
