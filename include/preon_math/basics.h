...
#pragma once
#ifndef PREONMATH_BASICS_H
#define PREONMATH_BASICS_H
...
namespace Preon
{
    namespace Math
    {
        ...
        template<> inline bool IsZero<float>(float val) { return std::abs(val) < FEps; }
        template<> inline bool IsZero<double>(double val) { return std::abs(val) < DEps; }
        template<> inline bool IsZero<int>(int val) { return val == 0; }

        // IsExactlyZero.
        template <typename T> inline bool IsExactlyZero(T val);
        template<> inline bool IsExactlyZero<float>(float val) { return std::abs(val) == 0.0f; }
        template<> inline bool IsExactlyZero<double>(double val) { return std::abs(val) == 0.0; }
        template<> inline bool IsExactlyZero<int>(int val) { return val == 0; }

        // AreEqual.
        template <typename T> inline bool AreEqual(T a, T b) { return IsZero<T>(a - b); }

        // AreExactlyEqual.
        template <typename T> inline bool AreExactlyEqual(T a, T b) { return IsExactlyZero<T>(a - b); }

        // Signum function.
        template <typename T> inline int Signum(T val);
        template<> inline int Signum<float>(float val) { return (val > 0.0f) - (val < 0.0f); }
        template<> inline int Signum<double>(double val) { return (val > 0.0) - (val < 0.0); }
        template<> inline int Signum<int>(int val) { return (val > 0) - (val < 0); }

        // DegToRad.
        template <typename T> inline T DegToRad(T val);
        template<> inline float DegToRad<float>(float degree) { return FDegreeToRadians * degree; }
        template<> inline double DegToRad<double>(double degree) { return DDegreeToRadians * degree; }

        // RadToDeg.
        template <typename T> inline T RadToDeg(T val);
        template<> inline float RadToDeg<float>(float radians) { return FRadiansToDegree * radians; }
        template<> inline double RadToDeg<double>(double radians) { return DRadiansToDegree * radians; }
