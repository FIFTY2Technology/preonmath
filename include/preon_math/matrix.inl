...
namespace Math
{
    ...
    template<PrMathSize M, PrMathSize N, typename T>
    inline vec<M, T>& matrix<M, N, T>::column(PrMathSize column)
    {
        return m_Data[column];
    }

    template<PrMathSize M, PrMathSize N, typename T>
    inline void matrix<M, N, T>::setColumn(PrMathSize column, const vec<M, T>& value)
    {
        m_Data[column] = value;
    }
    ...
    template<PrMathSize M, PrMathSize N, typename T>
    inline vec<N, T> matrix<M, N, T>::row(PrMathSize row) const
    {
        vec<N, T> value;
    ...
    template<PrMathSize M, PrMathSize N, typename T>
    inline void matrix<M, N, T>::setRow(PrMathSize row, const vec<N, T>& value)
    {
        for (PrMathSize d = 0; d < N; d++)
    ...
    T_Out matrix<M, N, T>::rowVecProduct(PrMathSize r, const vec<N, T_Vec>& vec) const
    {
