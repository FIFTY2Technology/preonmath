...
namespace Math
{
    ...
    namespace Simd
    {
        ...
        using simd_if_float = typename std::enable_if<std::is_same<scalar, float>::value, float_simd>::type;
        template<typename scalar>
        ...
        {
            (*out) = _mm_setr_ps(func(0), (numElements > 1) ? func(1) : fillValue, (numElements > 2) ? func(2) : fillValue, (numElements > 3) ? func(3) : fillValue);
        }
        template<class Getter>
        PREONMATH_FORCEINLINE void setScalars(double_simd* out, const Getter& func, unsigned int numElements, double fillValue)
        {
            (*out) = _mm_setr_pd(func(0), (numElements > 1) ? func(1) : fillValue);
        }
        ...
        PREONMATH_FORCEINLINE bool _testAllZeroFloat(const float_simd x) { return _mm_test_all_zeros(_mm_castps_si128(x), _mm_castps_si128(x)) != 0; }
        ...
        PREONMATH_FORCEINLINE float_simd round(const float_simd x) { return _mm_round_ps(x, _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC); }

        PREONMATH_FORCEINLINE double_simd round(const double_simd x) { return _mm_round_pd(x, _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC); }

        PREONMATH_FORCEINLINE float_simd max(float_simd a, float_simd b) { return _mm_max_ps(a, b); }

        PREONMATH_FORCEINLINE double_simd max(double_simd a, double_simd b) { return _mm_max_pd(a, b); }

        PREONMATH_FORCEINLINE float_simd min(float_simd a, float_simd b) { return _mm_min_ps(a, b); }

        PREONMATH_FORCEINLINE double_simd min(double_simd a, double_simd b) { return _mm_min_pd(a, b); }

        PREONMATH_FORCEINLINE float_simd sqrt(float_simd a) { return _mm_sqrt_ps(a); }

        PREONMATH_FORCEINLINE double_simd sqrt(double_simd a) { return _mm_sqrt_pd(a); }
...
PREONMATH_FORCEINLINE Preon::Math::float_simd operator+(Preon::Math::float_simd a, Preon::Math::float_simd b)
{
...
PREONMATH_FORCEINLINE Preon::Math::float_simd operator-(Preon::Math::float_simd a)
{
    return Preon::Math::Simd::xor_mask(a, Preon::Math::Simd::make<float>(-0.f));
}

PREONMATH_FORCEINLINE Preon::Math::double_simd operator+(Preon::Math::double_simd a, Preon::Math::double_simd b)
{
...
PREONMATH_FORCEINLINE Preon::Math::double_simd operator-(Preon::Math::double_simd a)
{
    return Preon::Math::Simd::xor_mask(a, Preon::Math::Simd::make<double>(-0.0));
}
