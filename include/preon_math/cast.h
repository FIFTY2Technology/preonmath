...
#ifndef PREONMATH_CAST_H
#define PREONMATH_CAST_H
...
namespace Preon
{
    namespace Math
    {
        ...
        {
            return euler<TOut>(static_cast<TOut>(e.phi()),
                               static_cast<TOut>(e.theta()),
                               static_cast<TOut>(e.psi()));
        }

        template<typename TOut, typename TIn>
        quat<TOut> cast(const quat<TIn>& q)
        {
            return quat<TOut>(static_cast<TOut>(q.scalar()),
                              static_cast<TOut>(q.x()),
                              static_cast<TOut>(q.y()),
                              static_cast<TOut>(q.z()));
        }

    }  // namespace Math
}  // namespace Preon

#endif // PREONMATH_CAST_H
