...
#pragma once
#ifndef PREONMATH_VEC_H
#define PREONMATH_VEC_H
...
namespace Preon
{
    namespace Math
    {
        ...
        private:
            ...
            template<typename E>
            void legacyInitZeroFunc(enable_if_non_simd<E, void*> = nullptr) { setZero(); }
            ...
            PREONMATH_FORCEINLINE static enable_if_non_simd<E, E> scalar_zero() { return E(0); }

            template<typename E = T>
            PREONMATH_FORCEINLINE static enable_if_simd<E, E> scalar_zero() { return Simd::zero<typename Simd::Scalar<T>::type>(); }
        ...
        public:
            ...
            template <size_t E = D> PREONMATH_FORCEINLINE vec(T x, T y, typename std::enable_if<E == 2, T>::type* = 0) { set(x, y);  }
            template <size_t E = D> PREONMATH_FORCEINLINE vec(T x, T y, T z, typename std::enable_if<E == 3, T>::type* = 0) {  set(x, y, z);  }
            template <size_t E = D> PREONMATH_FORCEINLINE vec(T x, T y, T z, T w, typename std::enable_if<E == 4, T>::type* = 0) {  set(x, y, z, w);  }
            ...
            template <size_t E = D> PREONMATH_FORCEINLINE vec(T a, T b, T c, T d, T e, typename std::enable_if<E == 5, T>::type* = 0) {  set(a, b, c, d, e);  }
            template <size_t E = D> PREONMATH_FORCEINLINE vec(T a, T b, T c, T d, T e, T f, typename std::enable_if<E == 6, T>::type* = 0) {  set(a, b, c, d, e, f);  }
        #else
            template <typename ...Args, typename = typename std::enable_if<sizeof...(Args) == D && ISGREATER(D, 4), void>::type>
            vec(Args&&... vals) : m_Data({{vals...}}) { }
            ...
            template <size_t E> PREONMATH_FORCEINLINE explicit vec(const vec<E, T>& v, T component, typename std::enable_if<E == D - 1, T>::type* = 0)
            {
            ...
            template <typename S> PREONMATH_FORCEINLINE explicit vec(const vec<D, S>& vec)
            {
            ...
            template <typename Lambda> PREONMATH_FORCEINLINE explicit vec(const Lambda& f, enable_if_not_convertible<Lambda, T> = 0)
            {
                setAll(f);
            }
            ...
            template <size_t E> typename std::enable_if<E <= D, vec<E, T> >::type subVector() const { return vec<E, T>([this](int i) -> const T& { return m_Data[i]; }); }
            ...
            template <typename ...Args, size_t E = D> typename std::enable_if<E == sizeof...(Args) && ISGREATER(E, 4), void>::type set(Args&&... vals) { m_Data = {{vals...}}; }

            void setElement(size_t d, T value) { m_Data[d] = value; }

            //! Sets all the elements to the given fill value
            void setAll(T fillValue) { return setAll([&] (size_t) { return fillValue; } ); }

            //! Sets the elements by evaluating the given lambda expression f(size_t d) -> T for all elements of the vector, indexed by d.
            template <typename Lambda> PREONMATH_FORCEINLINE void setAll(const Lambda& f, enable_if_not_convertible<Lambda, T> = 0)
            {
            ...
            vec<D, T> normalized() const { vec<D, T> out = (*this); out.normalize(); return out; }
            ...
            static vec<D, T> and_mask_scalar(const vec<D, T>& v1, const MaskType s)
            {
                return vec<D, T>([&] (size_t d) { return Simd::and_mask(v1[d], (T)s); });
            }
