...
#pragma once
#ifndef PREONMATH_SIMD_SCALAR_WRAPPER_H
#define PREONMATH_SIMD_SCALAR_WRAPPER_H
...
namespace Preon
{
    namespace Math
    {
        namespace SimdWrapper
        {
            ...
            {
            protected:
                Base(T_Scalar value, T_Simd valueSimd) : m_Value(value), m_ValueSimd(valueSimd) { }
            ...
            public:
                ...
                FloatingPoint(T_Scalar value) : Base<T_Scalar, T_Simd>(value, Simd::make<T_Scalar>(value)) { }
            ...
            template <> template <>
            inline float Float::get() const { return m_Value; }
            template <> template <>
            inline float_simd Float::get() const { return m_ValueSimd; }
            template <> template <>
            inline double Double::get() const { return m_Value; }
            template <> template <>
            inline double_simd Double::get() const { return m_ValueSimd; }
            ...
            {
            public:
                Vec3f(vec<3, float> value) : Base(value, vec<3, float_simd>(value)) { }
            ...
            template <>
            inline vec<3, float> Vec3f::get() const { return m_Value; }

            template <>
            inline vec<3, float_simd> Vec3f::get() const { return m_ValueSimd; }


            // Operators
            template <template <typename, typename> class WrapperType, typename T, typename = std::enable_if_t<std::is_convertible<WrapperType<T, typename Simd::Register<T>::type>, Base<T, typename Simd::Register<T>::type>>::value>>
            ...
            template <template <typename, typename> class WrapperType, typename T, typename = std::enable_if_t<std::is_convertible<WrapperType<T, typename Simd::Register<T>::type>, Base<T, typename Simd::Register<T>::type>>::value>, typename = std::enable_if_t<std::is_arithmetic<T>::value>>
            ...
            template <>
            inline float make<float>(float val) { return val; }
            template <>
            inline float_simd make<float_simd>(float val) { return Simd::make<float>(val); }
            template <>
            inline double make<double>(double val) { return val; }
            template <>
            inline double_simd make<double_simd>(double val) { return Simd::make<double>(val); }
            // also overload the following for convenience
            template <>
            inline float make<float>(double val) { return val; }
            template <>
            inline float_simd make<float_simd>(double val) { return Simd::make<float>(val); }
            ...
            template <>
            inline vec3f make<vec3f>(const vec3f& val) { return val; }

            template <>
            inline vec_simd<3> make<vec_simd<3>>(const vec3f& val) { return vec_simd<3>(val); }
...
#endif  // PREONMATH_SIMD_SCALAR_WRAPPER_H
